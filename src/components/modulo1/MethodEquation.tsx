import { InterpolationMethod, METHOD_LABELS } from "@/lib/types";
...
export default function MethodEquation({
  ...
  return (
    <details className="glass-panel rounded-xl">
      <summary className="px-4 py-3 cursor-pointer text-on-surface text-sm font-medium hover:text-primary-container transition-colors">
        📐 Equação do Método
      </summary>
      <div className="px-4 pb-4 space-y-3">
        <div className="text-primary-container text-sm font-semibold">{label}</div>
        <pre className="bg-surface-container-high/50 rounded-lg p-3 text-on-surface text-sm font-mono whitespace-pre-wrap overflow-x-auto leading-relaxed">
          {info.formula}
        </pre>
        <p className="text-outline text-sm leading-relaxed">{info.description}</p>
        {method === "smoothing-spline" && smoothingFactor !== undefined && (
          <p className="text-outline text-xs">
            Parâmetro de suavização atual:{" "}
            <span className="text-secondary font-mono">{smoothingFactor}</span>
          </p>
        )}
      </div>
    </details>
