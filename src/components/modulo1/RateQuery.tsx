...
export default function RateQuery({
  ...
  const interpolateAt = (bdays: number): number | null => {
    ...
    try {
      const result = applyMethod(method, xData, yData, smoothingFactor);
      // Find the closest smooth point or interpolate directly
      const target = [bdays];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="glass-panel p-4 rounded-lg">
        <h3 className="text-sm text-outline uppercase mb-3">
          Consultar por Data
        </h3>
        <div className="flex gap-2 items-end">
          <div className="flex-1">
            <label className="text-xs text-outline block mb-1">
              Data de Vencimento
            </label>
            <input
              type="date"
              ...
              className="w-full bg-surface-container-high border border-outline-variant rounded-md px-3 py-1.5 text-sm text-on-surface"
            />
          </div>
          <button
            type="button"
        ...
        {resultDateRate !== null && (
          <div className="mt-3 text-center">
            ...
            <div className="text-xs text-outline mt-1">Taxa interpolada</div>
          </div>
        )}
      </div>
